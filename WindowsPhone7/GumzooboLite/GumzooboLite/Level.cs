...
namespace BubbleGame
{
    public class Level
    {
        ...
        // dimensions
        static Rectangle backgroundSize = new Rectangle(56, 0, 800 - 56, 480 - 48);
        Rectangle backgroundSource = new Rectangle(0, 0, 0, 0);
        Color darkColor = new Color(0, 0, 0, 255);
        static Vector2 pointsVector = new Vector2(((800 - 56) / 2) + 56, 20);
            ...
            for (int y = 0; y < 17; y++)
            {
                for (int x = 0; x < 31; x++)
                {
                    ...
                    for (int x2 = x; x2 < x + 3; x2++)
                    {
                        ...
                        {
                            if (y2 >= 17 || x2 >= 31 || levellayout[y2].ToCharArray()[x2] != '-')
                            {
                                isWarpable = false;
                                break;
                            }
                        }
                        if (y+3 >= 17 || x2 >= 31 || levellayout[y+3].ToCharArray()[x2] != 'X')
                        {
                            isWarpable = false;
                            break;
                        }
                    ...
                    if (isWarpable)
                    {
                        OpenSpaces.Add(new Vector2(x * 24 + xoffset, y * 24 - 8));
                    }

                    char placement = levellayout[y].ToCharArray()[x];
                    if (placement == 'X')
                    {
                        AddSprite(new EnvironmentSprite(MapScreen.currentLocation.envTextureFrame, new Vector2(x * 24 + xoffset, y * 24 - 8)));
                    }
        ...
        public void Update(GameTime gameTime)
        {
            ...
            if (BubbleGame.IsTrialModeCached)
            {
            ...
            for (int currentIndex = 0; currentIndex < levelSprites.Count; currentIndex++)
            {
                ...
                if (sprite != null)
                    levelSprites[currentIndex].Update(gameTime);
            }
        ...
        public void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            ...
            // draw brackground
            spriteBatch.Draw(background, backgroundSize, backgroundSource,Color.White);
            ...
            if (MapScreen.currentLocation.locationIndex == 9)
            {
                ...
                {
                    darkColor.A = (Byte)(255 * ((float)currentDarkTime - (float)lightTime) / ((float)transToDark - (float)lightTime));
                }
                ...
                {
                    darkColor.A = (Byte)(255 - 255 * (((float)currentDarkTime - (float)darkTime) / ((float)transToLight - (float)darkTime)));
                }
                else
                {
                    darkColor.A = 0;
            ...
            Fonts.DrawCenteredText(spriteBatch, Fonts.HeaderFont, BubbleGame.players[0].PointsString, pointsVector, Color.White);
            ...
            if (isStarting && !isFailed)
            {
                ...
                spriteBatch.Draw(text, new Vector2(400 + 56/2 - text.Width / 2, 240 - 24 - text.Height / 2), Color.White);
            }
            else if (isComplete && !isFailed)
            {
                Texture2D complete = InternalContentManager.GetTexture("Complete");
                spriteBatch.Draw(complete, new Vector2(400 + 56 / 2 - complete.Width * .75f / 2, 240 - 24 - complete.Height / 2), null, Color.White, 0f, Vector2.Zero, .75f, SpriteEffects.None, 0);
            }
            else if (isFailed)
            {
                Texture2D gameover = InternalContentManager.GetTexture("GameOver");
                spriteBatch.Draw(gameover, new Vector2(400 + 56 / 2 - gameover.Width / 2, 240 - 24 - gameover.Height / 2), Color.White);
            }
        }
        ...
        public void ShowHelp()
        {
            ...
            if (levelNum == 0)
            {
                ...
                helpScreen = new HelpScreen("Controls");
            }
            else if (levelNum == 1)
            {
        ...
        public void FailLevel()
        {
            ...
            AudioManager.audioManager.PlaySFX("GameOver");
        }
