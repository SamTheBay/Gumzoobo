...
namespace BubbleGame
{
    class AviaryCut : CutSceneScreen
    {
        public AviaryCut()
        {
            ...
            pawns[1] = new Pawn("Tortoise", new Vector2(-100, 450));
            pawns[2] = new Pawn("Hunter", new Vector2(-100, 100));

            // setup events in the scene
            events = new CutSceneEvent[8];
            events[0] = new CutSceneEvent(pawns[2], 1000, CutSceneAction.Move, new Vector2(700, 450));
            events[1] = new CutSceneEvent(pawns[1], 1, CutSceneAction.Move, new Vector2(700, 450));
            events[2] = new CutSceneEvent(pawns[1], 5000, CutSceneAction.InShell);
            events[3] = new CutSceneEvent(pawns[0], 5000, CutSceneAction.Move, new Vector2(200, 450));
            events[4] = new CutSceneEvent(pawns[0], 7000, CutSceneAction.Move, new Vector2(-100, 450));
            events[5] = new CutSceneEvent(pawns[2], 7000, CutSceneAction.Move, new Vector2(-100, 450));
            events[6] = new CutSceneEvent(pawns[1], 10000, CutSceneAction.OutShell);
            events[7] = new CutSceneEvent(pawns[1], 11000, CutSceneAction.Move, new Vector2(1400, 450));
