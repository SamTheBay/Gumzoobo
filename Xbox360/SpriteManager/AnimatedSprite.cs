    ...
    public class AnimatedSprite : GameSprite
    {
        ...
        public virtual int CanMove(Direction direction, int velocity, List<GameSprite> env)
        {
            if (direction == Direction.Down)
            {
                ...
                if (bottomRectangle.Y + bottomRectangle.Height > 780)
                {
                    Rectangle transRect = new Rectangle(bottomRectangle.X, -60, bottomRectangle.Width, bottomRectangle.Y + bottomRectangle.Height - 780 + 50);
                    for (int i = 0; i < env.Count; i++)
                    {
                        if (env[i].IsActive == false)
                            continue;
            ...
            {
                if (position.X <= 208f + 24f - reduceSides / 2)
                {
                    return 0;
                }
            ...
            {
                if (position.X + frameDimensions.X + 24 - reduceSides/2 >= 1072f)
                {
                    return 0;
                }
