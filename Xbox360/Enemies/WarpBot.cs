...
namespace BubbleGame
{
    class WarpBot : Enemy
    {
        ...
        int invisibleTime = 5000;

        public WarpBot(Vector2 startPosition, Direction currentDirection)
            : base(startPosition, "Warpbot", new Point(40, 68), new Point(20, 34), 5, new Vector2(20f, 34f), currentDirection)
        {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            else if (actionTimer >= visibleTime)
            {
                ...
                if (startAudio == true)
                {
                    AudioManager.PlayCue("Invisable");
                    startAudio = false;
                }
                int flashtime = 100;
                if ((actionTimer / flashtime) % 2 == 0)
        ...
        private void Warp()
        {
            // change our position to an open space in the level
            position = Level.singletonLevel.GetOpenLocation();
        }
    }
}